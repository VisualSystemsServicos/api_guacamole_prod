...
def main(old_org, new_org, list_hosts, ip_guacd, token_guacamole=None, data_source=None):
    log_geral.info(f'')
    log_geral.info(f'############################################################################')
    log_geral.info(f'')
    ...
    try:
        ...
        if token_vault is not None or token_guacamole is not None:
            ...
            log_geral.info(f'({script_name}) - new_org - {new_org}')
            log_geral.info(f'({script_name}) - grupo - {grupo}')
            log_geral.info(f'({script_name}) - org_path_guacamole - {org_path_guacamole}')
            log_geral.info(f'({script_name}) - lista de hosts informada - {list_hosts}')
            ...
            for host in list_hosts:
                ...
                hosts_for_migrate.append(host_full)
            ...
            connections_filtred = [item for item in connections_group if item['name'] in list_hosts]

            clear_guacamole = delete_connections_guacamole(url_guacamole=url_guacamole, data_source=data_source, token=token_guacamole, list_connections=connections_filtred)

            # Cria a nova organização no Vault e Deleta a antiga
            create_new_org_vault = create_vault_new_org(url_vault=url_vault, token=token_vault, path_old_org=old_org, path_new_org=new_org, list_hosts=hosts_for_migrate)
            # Cria o caminho dentro do Guacamole
...
if __name__ == '__main__':
    """
        ...
        list_hosts = ['hitmongo01', '...']
        ip_guacd = '192.168.150.127'
    """

    main_result = main(old_org=old_org, new_org=new_org, list_hosts=list_hosts, ip_guacd=ip_guacd)
